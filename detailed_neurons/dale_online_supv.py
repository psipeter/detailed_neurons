...
class LearningNode(nengo.Node):
	def __init__(self, N_E, N_I, N_ens, w_E, w_I, dt=0.001, updates=1, k_E_pos=3e-9, k_E_neg=1e-8, k_I_pos=1e-7, k_I_neg=3e-9, seed=0):
		self.N_E = N_E
		self.N_I = N_I
		self.N_ens = N_ens
...
def integrate():
	...
	with nengo.Network(seed=seed) as network:
		...
		u = nengo.Node(nengo.processes.WhiteSignal(period=50, high=1, rms=0.5, seed=3))
		x = nengo.Ensemble(1, 1, neuron_type=nengo.Direct())
		node_E = LearningNode(N_E, N_I, N_E, w_EE, w_IE, dt=dt, k_E_pos=0e-8, k_E_neg=0e-7, k_I_pos=0e-7, k_I_neg=0e-8)
		node_I = LearningNode(N_E, N_I, N_I, w_EI, w_II, dt=dt, k_E_pos=0e-8, k_E_neg=0e-7, k_I_pos=0e-7, k_I_neg=0e-8)
		...
		# connections into E
		nengo.Connection(E.neurons, node_E[0:N_E], synapse=0.1)
		nengo.Connection(supv_I.neurons, node_E[N_E: N_E+N_I], synapse=0.01)
		nengo.Connection(E.neurons, node_E[N_E+N_I: N_E+N_I+N_E], synapse=0.1)
		nengo.Connection(supv_E.neurons, node_E[N_E+N_I+N_E: N_E+N_I+N_E+N_E], synapse=0.1)
		nengo.Connection(node_E, E.neurons, synapse=None)

		# connections into I
		nengo.Connection(supv_E.neurons, node_I[0:N_E], synapse=0.1)
		nengo.Connection(supv_I.neurons, node_I[N_E: N_E+N_I], synapse=0.01)
	...
	# w_IE = node_E.w_I
	# w_II = node_I.w_I
	# np.savez("data/dale_online_supv_integrate_train2.npz", w_EE=w_EE, w_EI=w_EI, w_IE=w_IE, w_II=w_II)
	...
	# 	ax3.set(ylabel='Firing Rate', ylim=((0, 40)), title='supv_I')
	# 	ax4.set(ylabel='Firing Rate', ylim=((0, 40)), title='I')
	# 	plt.savefig('plots/dale_online_supv_integrate_test_rates_%s'%chunk)
	# 	plt.close()
