...
class LearningNode(nengo.Node):
	def __init__(self, N_E, N_I, N_ens, w_E, w_I, updates=1, k_E_pos=3e-9, k_E_neg=1e-8, k_I_pos=1e-7, k_I_neg=3e-9, seed=0):
		self.N_E = N_E
		self.N_I = N_I
		self.N_ens = N_ens
...
def feedforward():
	...
	with nengo.Network(seed=seed) as network:
		...
		node = LearningNode(N_E, N_I, N_ens, w_E, w_I, k_E_pos=1e-8, k_E_neg=1e-7, k_I_pos=1e-7, k_I_neg=1e-8)

		pre = nengo.Ensemble(N_E, 1, seed=seed)
		E = nengo.Ensemble(N_E, 1, max_rates=nengo.dists.Uniform(20, 40), neuron_type=neuron_type, seed=seed)
		I = nengo.Ensemble(N_I, 1, max_rates=nengo.dists.Uniform(10, 30), neuron_type=neuron_type, seed=seed)
		...
		p_post_neurons = nengo.Probe(post.neurons, synapse=0.1)
		# p_supv = nengo.Probe(supv, synapse=0.05)
		p_supv_neurons = nengo.Probe(supv.neurons, synapse=0.1)
		p_x = nengo.Probe(x, synapse=0.1)
	...
	np.savez("data/dale_online_supv_feedforward_%s.npz"%neuron_type, w_E=w_E, w_I=w_I, d_post=d_post)
	...
	with nengo.Network(seed=seed) as network:
		...
		# p_E = nengo.Probe(E, synapse=0.05)
		# p_I = nengo.Probe(I, synapse=0.05)
		p_post = nengo.Probe(post, synapse=0.1, solver=nengo.solvers.NoSolver(d_post))
		# p_supv = nengo.Probe(supv, synapse=0.05)
		p_x = nengo.Probe(x, synapse=0.1)
		p_post_neurons = nengo.Probe(post.neurons, synapse=0.1)
		p_supv_neurons = nengo.Probe(supv.neurons, synapse=0.1)
	...
	ax2.set(ylabel='Firing Rate', ylim=((0, 200)), title='supervisor')
	ax3.set(ylabel='Firing Rate', ylim=((0, 200)), title='post')
	plt.savefig("plots/dale_online_supv_feedforward_%s.png"%neuron_type)
	plt.show()
...
def integrate():
	...
	N_post = 10
	seed = 1
	a = nengo.dists.Uniform(1, 10).sample(n=N_post, rng=np.random.RandomState(seed=seed))
	b = np.zeros((N_post))
	...
	with nengo.Network(seed=seed) as network:
		...
		node_E = LearningNode(N_E, N_I, N_E, w_EE, w_IE, k_E_pos=3e-9, k_E_neg=3e-8, k_I_pos=1e-7, k_I_neg=1e-9)
		node_I = LearningNode(N_E, N_I, N_I, w_EI, w_II, k_E_pos=3e-9, k_E_neg=3e-8, k_I_pos=1e-8, k_I_neg=1e-10)
		...
		post = nengo.Ensemble(N_post, 1, gain=a, bias=b, seed=seed)
		...
		nengo.Connection(E.neurons, node_E[N_E+N_I: N_E+N_I+N_E], synapse=0.05)
		...
		nengo.Connection(post.neurons, node_post[N_E+N_I: N_E+N_I+N_post], synapse=0.05)
		nengo.Connection(supv_post.neurons, node_post[N_E+N_I+N_post: N_E+N_I+N_post+N_post], synapse=0.05)
		nengo.Connection(node_post, post.neurons, synapse=None)
		conn = nengo.Connection(post, error_post, synapse=0.05, function=lambda x: 0, learning_rule_type=nengo.PES(learning_rate=3e-5))
		nengo.Connection(x, error_post, synapse=0.15, transform=-1)
		nengo.Connection(error_post, conn.learning_rule)

		# probes
		p_E = nengo.Probe(E, synapse=0.05)
		p_E_neurons = nengo.Probe(E.neurons, synapse=0.05)
		p_I = nengo.Probe(I, synapse=0.05)
		p_I_neurons = nengo.Probe(I.neurons, synapse=0.05)
		p_post = nengo.Probe(post, synapse=0.05)
	...
	# 	nengo.Connection(E.neurons, E.neurons, synapse=0.1, transform=w_EE.T, seed=seed)
	# 	nengo.Connection(E.neurons, I.neurons, synapse=0.1, transform=w_EI.T, seed=seed)
	# 	nengo.Connection(I.neurons, E.neurons, synapse=0.001, transform=w_IE.T, seed=seed)
	# 	nengo.Connection(I.neurons, I.neurons, synapse=0.001, transform=w_II.T, seed=seed)
	...
	# ax.set(ylabel='Firing Rate', ylim=((0, 400)), title='supv_E')
	# ax2.set(ylabel='Firing Rate', ylim=((0, 400)), title='E')
	# ax3.set(ylabel='Firing Rate', ylim=((0, 400)), title='supv_I')
	# ax4.set(ylabel='Firing Rate', ylim=((0, 400)), title='I')
	# ax5.set(ylabel='Firing Rate', ylim=((0, 400)), title='supv_post')
	# ax6.set(ylabel='Firing Rate', ylim=((0, 400)), title='post')
...
def integrate_2stage():
	...
	w_IE = np.zeros((N_I, N_E))
	w_II = np.zeros((N_I, N_I))
	# w_EE = np.load("data/dale_online_supv_integrate_2stage.npz")['w_EE']
	# w_EI = np.load("data/dale_online_supv_integrate_2stage.npz")['w_EI']
	# w_IE = np.load("data/dale_online_supv_integrate_2stage.npz")['w_IE']
	# w_II = np.load("data/dale_online_supv_integrate_2stage.npz")['w_II']

	with nengo.Network(seed=seed) as network:
		# nodes
		u = nengo.Node(nengo.processes.WhiteSignal(period=50, high=1, rms=0.5, seed=3))
		x = nengo.Ensemble(1, 1, neuron_type=nengo.Direct())
		node_E = LearningNode(N_E, N_I, N_E, w_EE, w_IE, k_E_pos=1e-8, k_E_neg=1e-7, k_I_pos=1e-7, k_I_neg=1e-8)
		node_I = LearningNode(N_E, N_I, N_I, w_EI, w_II, k_E_pos=1e-8, k_E_neg=1e-7, k_I_pos=1e-7, k_I_neg=1e-8)
	...
	# 	ax3.set(ylabel='Firing Rate', ylim=((0, 200)), title='supv_I')
	# 	ax4.set(ylabel='Firing Rate', ylim=((0, 200)), title='I')
	# 	plt.savefig('plots/dale_online_supv_integrate_2stage_train2_rates_%s'%chunk)
	# 	plt.close()
	...
	# w_IE = node_E.w_I
	# w_II = node_I.w_I
	# np.savez("data/dale_online_supv_integrate_2stage_train2.npz", w_EE=w_EE, w_EI=w_EI, w_IE=w_IE, w_II=w_II)
