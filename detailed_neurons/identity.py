...
def run(n_neurons=100, t=10, t_test=10, f=Lowpass(0.1), dt=0.001, n_tests=10, neuron_type=nengo.LIF(),
    ...
    if isinstance(neuron_type, DurstewitzNeuron):
        if load_w:
            ...
            data = go(d_ens, f_ens, n_neurons=n_neurons, t=t, f=Lowpass(0.1), dt=0.001, stim_func=stim_func,
                neuron_type=neuron_type, w_ens=w_ens, w_ens2=w_ens2, learn=True)
            w_ens = data['w_ens']
    ...
    if load_fd:
        ...
        data = go(d_ens, f_ens, n_neurons=n_neurons, t=t, f=f, dt=dt, neuron_type=neuron_type,
            stim_func=stim_func, w_ens=w_ens, w_ens2=w_ens2)
        print('optimizing filters and decoders')
        d_ens, f_ens, taus_ens = df_opt(data['tar'], data['ens'], f, order=2, df_evals=100, dt=dt, name='identity_%s'%id(neuron_type))
        np.savez('data/fd_identity_%s.npz'%neuron_type, d_ens=d_ens, taus_ens=taus_ens)
        ...
        plt.tight_layout()
        plt.savefig("plots/identity_filters_%s.png"%neuron_type)
        ...
        plt.savefig("plots/identity2_train1_%s.png"%neuron_type)
    ...
    if load_fd:
        ...
        d_ens2, f_ens2, taus_ens2  = df_opt(data['tar2'], data['ens2'], f, order=2, df_evals=100, dt=dt, name='identity2_%s'%id(neuron_type))
        np.savez('data/fd_identity_%s.npz'%id(neuron_type), d_ens=d_ens, taus_ens=taus_ens, d_ens2=d_ens2, taus_ens2=taus_ens2)
        ...
        ax.legend(loc='upper right')
        plt.tight_layout()
        plt.savefig("plots/identity_filters2_%s.png"%neuron_type)
        ...
        plt.legend(loc='upper right')
        plt.savefig("plots/identity2_train2_%s.png"%neuron_type)
    ...
    for test in range(n_tests):
        ...
        plt.legend(loc='upper right')
        plt.savefig("plots/identity2_test_%s_%s.png"%(test, neuron_type))
        ...
        plt.legend(loc='upper right')
        plt.savefig("plots/identity2_test2_%s_%s.png"%(test, neuron_type))
    ...
    np.savez('data/results_identity2_%s.npz'%neuron_type,
...
# plt.tight_layout()
# plt.savefig("plots/identity2_nrmses.png")