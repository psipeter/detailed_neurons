...
def go(d_ens, f_ens, n_neurons=30, n_pre=300, t=10, m=Uniform(20, 40), i=Uniform(-1, 0.6), seed=0, dt=0.001, T=0.2, neuron_type=LIF(), f=Lowpass(0.2), f_smooth=DoubleExp(1e-3, 2e-1), stim_func=lambda t: np.sin(t), stim_func2=None, w_x=None, e_x=None, w_u=None, e_u=None, w_fb=None, e_fb=None, L_x=False, L_u=False, L_fd=False, L_fb=False, supervised=False):
...
#                 ax2.set(xlabel='time', ylabel='firing rate', ylim=((0, 40)))
#                 plt.legend()
#                 plt.savefig('plots/tuning/integrate_test_%s_activity_%s.pdf'%(test, n))
#                 plt.close('all')
...
#                 ax.plot(data['times'], data['v'][:,n])
#                 ax.set(xlabel='time', ylabel='voltage')
#                 plt.savefig('plots/tuning/integrate_test_%s_voltage_%s.pdf'%(test, n))
#                 plt.close('all')
...
# nrmses_durstewitz = run(neuron_type=DurstewitzNeuron(0.0), t_test=5, n_tests=1, supervised=False, load_w_u=True, load_w_x=True, load_w_fb=True, load_w="data/integrate_w.npz", load_fd="data/integrate_DurstewitzNeuron()_fd.npz")
