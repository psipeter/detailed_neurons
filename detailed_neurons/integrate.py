...
def make_normed_flipped(value=1.0, t=10.0, dt=0.001, N=10, f=Lowpass(0.01), normed='x', seed=0):
...
def go(d_ens, f_ens, n_neurons=30, n_pre=300, t=10, m=Uniform(30, 40), i=Uniform(-1, 0.6), seed=0, dt=0.001, T=0.2, neuron_type=LIF(), f=Lowpass(0.2), f_smooth=DoubleExp(1e-2, 2e-1), stim_func=lambda t: np.sin(t), stim_func_L_u=None, w_x=None, e_x=None, w_u=None, e_u=None, w_fb=None, e_fb=None, L_x=False, L_u=False, L_fd=False, L_fb=False, supervised=False):
...
def run(n_neurons=30, t=10, t_test=10, dt=0.001, n_trains=20, n_encodes=40, n_tests=20, neuron_type=LIF(), f=DoubleExp(1e-3, 2e-1), load_w=None, load_fd=None, load_w_x=False, load_w_u=False, load_w_fb=False, reg=1e-1, penalty=0.0, T=0.2, supervised=False):
    ...
    if load_fd:
        ...
        for ntrn in range(n_trains):
            ...
            stim_func = make_normed_flipped(value=1.0, t=t, dt=dt, N=1, f=f, normed='x', seed=ntrn)
...
#                 plt.legend()
#                 plt.savefig('plots/tuning/integrate_supervised_%s_activity_%s.pdf'%(test, n))
#                 plt.close('all')
...
#                 ax2.set(xlabel='time', ylabel='firing rate', ylim=((0, 40)))
#                 plt.legend()
#                 plt.savefig('plots/tuning/integrate_supervised_%s_activity2_%s.pdf'%(test, n))
#                 plt.close('all')
...
#                 plt.savefig('plots/tuning/integrate_supervised_%s_voltage_%s.pdf'%(test, n))
#                 plt.close('all')
...
# nrmses_durstewitz = run(neuron_type=DurstewitzNeuron(0.0), n_tests=1, supervised=False, load_w_u=True, load_w_x=True, load_w_fb=True, load_w="data/integrate_w.npz", load_fd="data/integrate_DurstewitzNeuron()_fd.npz")
