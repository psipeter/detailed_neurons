...
def go(NPre=100, N=100, t=10, m=Uniform(30, 30), i=Uniform(-0.8, 0.8), seed=0, dt=0.001, fPre=None, fBio=None, fS=DoubleExp(1e-3, 1e-1), dPreA=None, dPreB=None, dBio=None, ePreA=None, ePreB=None, eBio=None, stage=None, alpha=1e-7, eMax=1e-1, stimA=lambda t: np.sin(t), stimB=lambda t: 0):
    ...
    with nengo.Network(seed=seed) as model:
        ...
        ens = nengo.Ensemble(N, 1, max_rates=m, intercepts=i, neuron_type=Bio("Pyramidal"), seed=seed)
        ...
        if stage==4:
            ...
            ens2 = nengo.Ensemble(N, 1, max_rates=m, intercepts=i, neuron_type=Bio("Pyramidal"), seed=seed)
            ens3 = nengo.Ensemble(N, 1, max_rates=m, intercepts=i, neuron_type=Bio("Pyramidal"), seed=seed)
            nengo.Connection(inptB, preInptC, synapse=fBio, seed=seed)
            c1a = nengo.Connection(preInptA, ens, synapse=fPre, solver=NoSolver(dPreA), seed=seed)
            c2b = nengo.Connection(preInptB, ens2, synapse=fPre, solver=NoSolver(dPreB), seed=seed+1)
            c3 = nengo.Connection(ens2, ens, synapse=fBio, solver=NoSolver(dBio), seed=seed)
            ...
            learnEncoders(c3, ens3, fS, alpha=alpha, eMax=eMax)
            pTarEns = nengo.Probe(ens3.neurons, synapse=None)
        if stage==5:
            c1a = nengo.Connection(preInptA, ens, synapse=fPre, solver=NoSolver(dPreA), seed=seed)
            c5 = nengo.Connection(ens, ens, synapse=fBio, solver=NoSolver(dBio), seed=seed)
...
def run(NPre=100, N=100, t=10, nTrain=10, nTest=3, nEnc=10, dt=0.001, fPre=DoubleExp(1e-3, 1e-1), fS=DoubleExp(1e-3, 1e-1), Tff=0.3, reg=1e-1, tauRiseMax=3e-3, tauFallMax=3e-1, load=[], file=None):
    ...
    if 4 in load:
        ...
        fBio = DoubleExp(tauRiseBio, tauFallBio)
    else:
        ...
        dBio, fBio, tauRiseBio, tauFallBio, X2, Y2, error2 = decode(spikes, targets, nTrain, dt=dt, reg=reg, tauRiseMax=tauRiseMax, tauFallMax=tauFallMax, name="integrateNMDA")
#         dBio, X2, Y2, error2 = decodeNoF(spikes, targets, nTrain, f, dt=0.001, reg=reg)
    ...
    if 5 in load:
        ...
        for n in range(nEnc):
            stimA, stimB = makeSignal(t, fPre, dt=0.001, seed=n)
            data = go(dPreA=dPreA, dPreB=dPreB, dBio=dBio, ePreA=ePreA, ePreB=ePreB, eBio=eBio, NPre=NPre, N=N, t=t, dt=dt, fPre=fPre, fBio=fBio, fS=fS, stimA=stimA, stimB=stimB, stage=4, alpha=1e-7)
            eBio = data['eBio']
    ...
    for test in range(nTest):
        ...
        data = go(dPreA=dPreA, dBio=dBio, ePreA=ePreA, eBio=eBio, NPre=NPre, N=N, t=t, dt=dt, fPre=fPre, fBio=fBio, fS=fS, stimA=stimA, stimB=stimB, stage=5)
        A = fBio.filt(data['ens'], dt=dt)
        X = np.dot(A, dBio)
...
errors = run(N=30, nTrain=10, nEnc=10, load=[1,2,3], nTest=10, file="data/integrateNMDA.npz")
