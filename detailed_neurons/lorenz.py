...
def go(d_ens, f_ens, n_neurons=5000, t=30, learn=False, neuron_type=nengo.LIF(),
       m=Uniform(20, 40), i=Uniform(-0.8, 0.8), r=40, IC=[0,0,0],
       seed=0, dt=0.001, f=Lowpass(0.1)):
    ...
    with nengo.Network(seed=seed) as model:
        ...
        if learn:
            ...
            supv_ens = nengo.Connection(supv, ens, synapse=f, label='supv_ens', seed=seed)
        ens_ens = nengo.Connection(ens, ens, synapse=f_ens, solver=NoSolver(d_ens), label='ens_ens', seed=seed)
        # probes
...
def run(n_neurons=5000, neuron_type=nengo.LIF(), t_train=50, t=100, f=Lowpass(0.1), dt=0.001, seed=0,
        m=Uniform(20, 40), i=Uniform(-0.8, 0.8), r=40, n_tests=3, smooth=100,
    ...
    if load_fd:
        ...
        with nengo.Network(seed=seed) as model:
            ...
            ens_ens = nengo.Connection(ens, ens, synapse=f, function=feedback_NEF, seed=seed)
        with nengo.Simulator(model, dt=dt, seed=seed) as sim:
            d_nef = sim.data[ens_ens].weights.T
        d_ens = d_nef
        f_ens = f
    else:
        ...
        IC /= norm(IC, 1)
        data = go(d_ens, f_ens, neuron_type=neuron_type, n_neurons=n_neurons, IC=IC, r=r, learn=True, t=t_train, f=f, m=m, i=i, dt=dt, seed=seed)
        d_ens, f_ens, taus_ens = df_opt(data['x'], data['ens'], f, order=2, df_evals=100, dt=dt, name='lorenz_%s'%neuron_type)
        np.savez('data/fd_lorenz_%s.npz'%neuron_type, d_ens=d_ens, taus_ens=taus_ens)
        ...
        xhat_ens = np.dot(f_ens.filt(a_ens, dt=dt), d_ens)

        fig, ax = plt.subplots(figsize=((12, 8)))
        ax.plot(data['times'], target, linestyle="--", label='target')
        ax.plot(data['times'], xhat_ens, label='%s' %neuron_type)
    ...
    for n in range(n_tests):
        ...
        data = go(d_ens, f_ens, neuron_type=neuron_type, n_neurons=n_neurons, IC=IC, r=r,
            t=t, f=f, m=m, i=i, dt=dt, seed=seed, learn=False)
        times = data['times']
        ...
        ax.set(xlabel=r'$\mathrm{max}_n (z)$', ylabel=r'$\mathrm{max}_{n+1} (z)$', title='error = %.5f'%error)
        plt.legend(loc='upper right')
        plt.savefig("plots/lorenz_test_%s_tent_%s.png"%(test, neuron_type))
    ...
    np.savez('data/nrmses_lorenz_%s.npz'%neuron_type, nrmses=nrmses)
    return nrmses

nrmses_nef = run(neuron_type=nengo.SpikingRectifiedLinear(), r=35, NEF=True)
nrmses_lif = run(neuron_type=nengo.LIF(), load_fd="data/fd_lorenz_LIF().npz")
nrmses_alif = run(neuron_type=AdaptiveLIFT(tau_adapt=0.1, inc_adapt=0.1), load_fd="data/fd_lorenz_AdaptiveLIFT().npz")
nrmses_wilson = run(neuron_type=WilsonEuler(), dt=0.001, load_fd="data/fd_lorenz_WilsonEuler().npz")

nrmses = np.vstack((nrmses_nef, nrmses_lif, nrmses_alif, nrmses_wilson))
nt_names =  ['NEF', 'LIF', 'ALIF', 'Wilson']
fig, ax = plt.subplots(1, 1, figsize=(6, 6))
sns.barplot(data=nrmses.T)
ax.set(ylabel='NRMSE')
plt.xticks(np.arange(len(nt_names)), tuple(nt_names), rotation=0)
plt.tight_layout()
plt.savefig("plots/lorenz_nrmses.png")