...
def go(d_ens, f_ens, n_neurons=100, t=10, m=Uniform(20, 40), i=Uniform(-1, 0.8), seed=1, dt=0.001,
        f=Lowpass(0.01), f_smooth=Lowpass(0.1), stim_func=lambda t: np.sin(t), stim_func2=lambda t: np.sin(t),
        neuron_type=LIF(), w_ens=None, w_ens2=None, learn=False, learn2=False, half=False):
    ...
    with nengo.Network(seed=seed) as model:
        ...
        # Ensembles
        pre = nengo.Ensemble(200, 2, radius=2, max_rates=m, seed=seed)
        ...
        if learn and isinstance(neuron_type, DurstewitzNeuron):
            node = LearningNode(n_neurons, pre.n_neurons, 2, conn, k=2e-5)
        ...
        if learn2 and isinstance(neuron_type, DurstewitzNeuron):
            node2 = LearningNode(30, n_neurons, 1, conn2, k=2e-5)
...
def run(n_neurons=60, t=100, t_test=10, t_enc=400,
        f=DoubleExp(1e-3, 3e-2), f_out=DoubleExp(1e-3, 1e-1), f_smooth=DoubleExp(1e-2, 2e-1),
        dt=0.001, n_tests=10, neuron_type=LIF(), reg=0, penalty=1.0, load_w=None, load_df=None):
    ...
    if isinstance(neuron_type, DurstewitzNeuron):
        if load_w:
            ...
            stim_func = nengo.processes.WhiteSignal(period=t_enc, high=0.5, rms=0.6, seed=0)
            stim_func2 = nengo.processes.WhiteSignal(period=t_enc, high=0.5, rms=0.6, seed=1)
            data = go(d_ens, f_ens, n_neurons=n_neurons, t=t_enc, f=f, dt=0.001, stim_func=stim_func, stim_func2=stim_func2,
    ...
    if load_df:
        ...
        stim_func = nengo.processes.WhiteSignal(period=t, high=0.5, rms=0.6, seed=0)
        stim_func2 = nengo.processes.WhiteSignal(period=t, high=0.5, rms=0.6, seed=1)
        ...
        fig, ax = plt.subplots()
        ax.plot(data['times'], x_out, linestyle="--", label='x')
    ...
    stim_func = nengo.processes.WhiteSignal(period=t_enc, high=0.5, rms=0.6, seed=0)
    stim_func2 = nengo.processes.WhiteSignal(period=t_enc, high=0.5, rms=0.6, seed=1)
    data = go(d_ens, f_ens, n_neurons=n_neurons, t=t_enc, f=f, f_smooth=f_smooth, neuron_type=neuron_type,
    ...
    if load_df:
        ...
        stim_func2 = nengo.processes.WhiteSignal(period=t, high=0.5, rms=0.6, seed=1)
        data = go(d_ens, f_ens, n_neurons=n_neurons, t=t, f=f, dt=dt, neuron_type=neuron_type,
            stim_func=stim_func, stim_func2=stim_func2, w_ens=w_ens, w_ens2=w_ens2)
    ...
    for test in range(n_tests):
        print('test %s' %test)
        stim_func = nengo.processes.WhiteSignal(period=t_test, high=0.5, rms=0.6, seed=100+test)
        stim_func2 = nengo.processes.WhiteSignal(period=t_test, high=0.5, rms=0.6, seed=200+test)
...
nrmses_durstewitz = run(neuron_type=DurstewitzNeuron(), load_w="data/multiply_w.npz", load_df="data/multiply_DurstewitzNeuron()_df.npz")
...
# plt.savefig("plots/multiply_all_nrmses.pdf")