...
def go(d_ens, f_ens, n_neurons=100, t=10, m=Uniform(20, 40), i=Uniform(-1, 1), seed=0, dt=0.001, f=Lowpass(0.025), f_out=Lowpass(0.1),
    ...
    with nengo.Network(seed=seed) as model:
        ...
        # Ensembles
        pre = nengo.Ensemble(n_neurons, 2, radius=2, max_rates=m, seed=seed)
        ...
        supv = nengo.Ensemble(n_neurons, 2, radius=2, max_rates=m, intercepts=i, neuron_type=AdaptiveLIFT(), seed=seed)
        ...
        if learn and isinstance(neuron_type, DurstewitzNeuron):
            node = LearningNode(n_neurons, pre.n_neurons, 2, conn, k=1e-3)
...
def run(n_neurons=100, t=10, t_test=10, t_enc=100, f=Lowpass(0.025), f_out=Lowpass(0.1), dt=0.001, n_tests=10, neuron_type=nengo.LIF(), load_w=None, load_fd=None, learn_w=True, learn_w2=True):
    ...
    if load_fd:
        ...
        d_out1, f_out1, taus_out1 = df_opt(data['tar'], data['ens'], f_out, order=2, dt=dt, name='multiply_out1_%s'%neuron_type)
        np.savez('data/fd_multiply_%s.npz'%neuron_type, d_ens=d_ens, taus_ens=taus_ens, d_out1=d_out1, taus_out1=taus_out1)
        ...
        plt.tight_layout()
        plt.savefig("plots/multiply_filters_out1_%s.png"%neuron_type)
        plt.close()
        ...
        ax.legend(loc='upper right')
        plt.tight_layout()
        plt.savefig("plots/multiply_filters_ens_%s.png"%neuron_type)
        plt.close()
        ...
        if isinstance(neuron_type, DurstewitzNeuron):
            a_durstewitz = f_out1.filt(data['ens'], dt=0.001)
    ...
    if load_fd:
        ...
        d_out2, f_out2, taus_out2  = df_opt(data['tar2'], data['ens2'], f_out, order=2, dt=dt, name='multiply_%s'%neuron_type)
        np.savez('data/fd_multiply_%s.npz'%neuron_type,
            d_ens=d_ens, taus_ens=taus_ens, d_out1=d_out1, taus_out1=taus_out1, d_out2=d_out2, taus_out2=taus_out2)

        times = np.arange(0, 1, 0.0001)
        fig, ax = plt.subplots(figsize=((6, 6)))
        ax.plot(times, f_out.impulse(len(times), dt=0.0001), label="f")
        ...
        if isinstance(neuron_type, DurstewitzNeuron) and learn_w2:
            a_durstewitz = f_out2.filt(data['ens2'], dt=0.001)
            a_supv = f_out.filt(data['supv2'], dt=0.001)
            u = f.filt(data['tar2'], dt=0.001)
            enc = data['enc2']
            xbins = 40
            xmin = -1
            xmax = 1
...
# nrmses_durstewitz = run(n_neurons=100, t=100, t_enc=100, n_tests=1, neuron_type=DurstewitzNeuron())
nrmses_lif = run(n_neurons=100, t=100, n_tests=1, neuron_type=LIF())
# nrmses_alif = run(n_neurons=100, t=100, n_tests=1, neuron_type=AdaptiveLIFT())
# nrmses_wilson = run(n_neurons=100, t=100, n_tests=1, dt=0.000025, neuron_type=WilsonEuler())
