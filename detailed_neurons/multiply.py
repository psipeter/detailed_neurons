...
def go(d_ens, f_ens, n_neurons=100, t=10, m=Uniform(20, 40), i=Uniform(-0.8, 0.8), seed=0, dt=0.001, f=Lowpass(0.1),
        neuron_type=nengo.LIF(), w_ens=None, w_ens2=None, learn=False, learn2=False, stim_func=lambda t: np.sin(t), stim_func2=lambda t: np.sin(t)):
    ...
    with nengo.Network(seed=seed) as model:
        ...
        # Ensembles
        pre = nengo.Ensemble(200, 2, max_rates=m, intercepts=i, seed=seed)
        nef = nengo.Ensemble(n_neurons, 2, max_rates=m, intercepts=i, neuron_type=nengo.LIF(), seed=seed)
        ...
        nengo.Connection(nef, nef2, synapse=f, function=multiply, seed=seed+1, label='nef-nef2')
        if isinstance(neuron_type, DurstewitzNeuron):
            # todo: fix nosolver bug
        ...
        nengo.Connection(tar, tar_mult, synapse=None, function=multiply, seed=seed+1, label='tar-tar_mult')
        nengo.Connection(tar, tar2, synapse=f, function=multiply, seed=seed+1, label='tar-tar2')

        if learn and isinstance(neuron_type, DurstewitzNeuron):
            node = LearningNode(n_neurons, pre.n_neurons, 2, conn)
            ...
            nengo.Connection(nef.neurons, node[pre.n_neurons+n_neurons: pre.n_neurons+2*n_neurons], synapse=f)
            nengo.Connection(u, node[-2], synapse=f)
            nengo.Connection(u2, node[-1], synapse=f)
            
        if learn2 and isinstance(neuron_type, DurstewitzNeuron):
            node2 = LearningNode(n_neurons, n_neurons, 2, conn2, k=1e-2)
            ...
            nengo.Connection(tar[0], node2[-2], synapse=f)
            nengo.Connection(tar[1], node2[-1], synapse=f)
        ...
        p_nef2_neurons = nengo.Probe(nef2.neurons, synapse=None)
    ...
    return dict(
        ...
        nef2_neurons=sim.data[p_nef2_neurons],
...
def run(n_neurons=100, t=10, t_test=10, f=Lowpass(0.1), dt=0.001, n_tests=10, neuron_type=nengo.LIF(),
    ...
    if isinstance(neuron_type, DurstewitzNeuron):
        if load_w:
            ...
            data = go(d_ens, f_ens, n_neurons=n_neurons, t=t, f=Lowpass(0.1), dt=0.001, stim_func=stim_func, stim_func2=stim_func2, neuron_type=neuron_type, w_ens=w_ens, w_ens2=w_ens2, learn=True)
            w_ens = data['w_ens']
    ...
    if load_fd:
        ...
        d_ens_out, f_ens_out, taus_ens_out = df_opt(data['tar'], data['ens'], f, order=2, df_evals=100, dt=dt, name='multiply_readout_%s'%neuron_type)
        d_ens, f_ens, taus_ens = df_opt(data['tar_mult'], data['ens'], f, order=2, df_evals=100, dt=dt, reg=1e-2, name='multiply_%s'%neuron_type)
        ...
        xhat_nef_out = data['nef']
        xhat_ens_out = np.dot(a_ens_out, d_ens_out)
        nrmse_nef_out = nrmse(xhat_nef_out, target=target_out)
        ...
        fig, ax = plt.subplots(figsize=((6, 6)))
        ax.plot(data['times'], target_out, linestyle="--", label='target')
        ax.plot(data['times'], xhat_nef_out, label='nef, nrmse=%.3f' %nrmse_nef_out)
        ax.plot(data['times'], xhat_ens_out, label='ens, nrmse=%.3f' %nrmse_ens_out)
    ...
    # else:
        ...
        data = go(d_ens, f_ens, n_neurons=n_neurons, t=t, f=Lowpass(0.1), dt=0.001, neuron_type=neuron_type,
            stim_func=stim_func, stim_func2=stim_func2, w_ens=w_ens, w_ens2=w_ens2, learn2=True)
        fig, (ax, ax2) = plt.subplots(2, 1, figsize=(18, 18), sharex=True)
        ax.plot(data['times'], f.filt(data['nef2_neurons'], dt=dt)[:,:5], label='nef2')
        ax2.plot(data['times'], f.filt(data['ens2'], dt=dt)[:,:5], label='ens2')
        ax.set(ylabel="hz", title='supv')
        ax2.set(ylabel="hz", title='ens', xlabel='time (s)')
        plt.savefig("plots/w_ens2_tuning_0_5.png")
        fig, (ax, ax2) = plt.subplots(2, 1, figsize=(18, 18), sharex=True)
        ax.plot(data['times'], f.filt(data['nef2_neurons'], dt=dt)[:,5:10], label='nef2')
        ax2.plot(data['times'], f.filt(data['ens2'], dt=dt)[:,5:10], label='ens2')
        plt.savefig("plots/w_ens2_tuning_5_10.png")
        w_ens2 = data['w_ens2']
    ...
    if load_fd:
        ...
        d_ens2, f_ens2, taus_ens2  = df_opt(data['tar2'], data['ens2'], f, order=2, df_evals=100, dt=dt, name='multiply2_%s'%neuron_type)
        np.savez('data/fd_multiply_%s.npz'%neuron_type, d_ens=d_ens, taus_ens=taus_ens, d_ens2=d_ens2, taus_ens2=taus_ens2)
    ...
    print('nrmses: ', nrmses_nef2, nrmses_ens2)
    print('means: ', mean_nef2, mean_ens2)
    print('confidence intervals: ', CI_nef2, CI_ens2)
...
# nrmses_nef, nrmses_lif = run(n_neurons=100, t=10, n_tests=3, neuron_type=nengo.LIF())
# _, nrmses_alif = run(n_neurons=100, t=100, n_tests=3, neuron_type=AdaptiveLIFT(tau_adapt=0.1, inc_adapt=0.1))
# _, nrmses_wilson = run(n_neurons=100, t=100, n_tests=3, dt=0.001, neuron_type=WilsonEuler())
_, nrmses_durstewitz = run(n_neurons=100, t=30, n_tests=0, dt=0.001, neuron_type=DurstewitzNeuron(),
    load_fd="data/fd_multiply_DurstewitzNeuron().npz", load_w="data/w_multiply.npz")
