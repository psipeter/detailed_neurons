...
def go(d_ens, f_ens, n_neurons=100, t=10, m=Uniform(20, 40), i=Uniform(-0.8, 0.8), seed=0, dt=0.001, f=Lowpass(0.1),
    ...
    with nengo.Network(seed=seed) as model:
        ...
        if learn2 and isinstance(neuron_type, DurstewitzNeuron):
            node2 = LearningNode(n_neurons, n_neurons, 2, conn2)
...
def run(n_neurons=100, t=10, t_test=10, f=Lowpass(0.1), dt=0.001, n_tests=10, neuron_type=nengo.LIF(), load_fd=None, load_fd2=None):
    ...
    w_ens = None
    w_ens2 = None
    stim_func = nengo.processes.WhiteSignal(period=t, high=1, rms=0.7, seed=0)
    stim_func2 = nengo.processes.WhiteSignal(period=t, high=1, rms=0.7, seed=1)
    ...
    if load_fd:
        ...
        d_ens, f_ens, taus_ens = df_opt(data['tar'][:,0]*data['tar'][:,1], data['ens'], f, order=2, df_evals=100, dt=dt, name='multiply_%s'%neuron_type)
    ...
    target = f.filt(data['tar'][:,0]*data['tar'][:,1], dt=dt)
    ...
    plt.savefig("plots/multiply_train1_readout_%s.png"%neuron_type)
    ...
    ax.set(xlabel='time (s)', ylabel=r'$\mathbf{x}$', title="train1")
    plt.legend(loc='upper right')
    plt.savefig("plots/multiply_train1_%s.png"%neuron_type)

    if isinstance(neuron_type, DurstewitzNeuron):
        print('optimizing encoders into DurstewitzNeuron ens2')
        # stim_func = lambda t: np.sin(t)
        data = go(d_ens, f_ens, n_neurons=n_neurons, t=t, f=Lowpass(0.1), dt=0.001, neuron_type=neuron_type, stim_func=stim_func, stim_func2=stim_func2, w_ens=w_ens, w_ens2=w_ens2, learn2=True)
        w_ens2 = data['w_ens2']
    ...
    ax.legend(loc='upper right')
    plt.tight_layout()
    plt.savefig("plots/multiply_filters2_%s.png"%neuron_type)
    ...
    plt.legend(loc='upper right')
    plt.savefig("plots/multiply_train2_%s.png"%neuron_type)
    ...
    for test in range(n_tests):
        ...
        ax.set(xlabel='time (s)', ylabel=r'$\mathbf{x}$', title="train1")
        plt.legend(loc='upper right')
        plt.savefig("plots/multiply_test1_readout_%s.png"%neuron_type)
        ...
        ax.set(xlabel='time (s)', ylabel=r'$\mathbf{x}$', title="train1")
        plt.legend(loc='upper right')
        plt.savefig("plots/multiply_test1_%s_%s.png"%(test, neuron_type))
        ...
        ax.set(xlabel='time (s)', ylabel=r'$\mathbf{x}$', title="train2")
        plt.legend(loc='upper right')
        plt.savefig("plots/multiply_test2_%s_%s.png"%(test, neuron_type))
        plt.close()
...
nrmses_nef, nrmses_lif = run(n_neurons=100, t=50, n_tests=3, neuron_type=nengo.LIF())
